...
def make_spatial_grid(
    ...
    *,
    ...
    """Return meshgrid (x, y) in mm with center at zero."""
...
def make_frequency_grid(
    ...
    *,
    ...
    """Return frequency-domain meshgrid (fx, fy) in cycles/mm."""
    dev = torch.device(device) if device is not None else torch.device("cpu")
    f = torch.fft.fftfreq(N, d=float(dx_mm), device=dev, dtype=dtype)
    if shifted:
