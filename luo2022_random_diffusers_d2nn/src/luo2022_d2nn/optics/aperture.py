"""Aperture masks."""

from __future__ import annotations

import torch

from luo2022_d2nn.optics.grids import make_frequency_grid, make_spatial_grid


def circular_aperture(
    N: int,
    dx_mm: float,
    radius_mm: float,
    *,
    device: torch.device | str | None = None,
) -> torch.Tensor:
    """Real-valued circular aperture mask in spatial domain."""
    x, y = make_spatial_grid(N=N, dx_mm=dx_mm, device=device, dtype=torch.float64)
    return ((x**2 + y**2) <= float(radius_mm) ** 2).to(torch.float32)


def na_mask(
    N: int,
    dx_mm: float,
    wavelength_mm: float,
    na: float,
    *,
    shifted: bool = False,
    device: torch.device | str | None = None,
) -> torch.Tensor:
    """Frequency-domain mask defined by numerical aperture cutoff."""
    fx, fy = make_frequency_grid(N=N, dx_mm=dx_mm, shifted=shifted, device=device, dtype=torch.float64)
    cutoff = float(na) / float(wavelength_mm)
    return ((fx**2 + fy**2) <= cutoff**2).to(torch.float32)
