...
function normalizeCoord(value) {
  ...
  // Some pptxgenjs objects such as tables store coordinates in EMU.
  return value > 1000 ? value / 914400 : value;
}
...
function compareElementPosition(slide, firstIndex, secondIndex) {
  ...
  const getBounds = (obj) => {
    ...
    let x = normalizeCoord(source.x);
    let y = normalizeCoord(source.y);
    let w = normalizeCoord(source.w);
    let h = normalizeCoord(source.h);
