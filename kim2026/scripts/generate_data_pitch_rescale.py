#!/usr/bin/env python
"""Production: delta_n=150μm + pitch rescale (NO interpolation).

Physical beam reducer simulation: crop 1024×1024 at 150μm, redefine dx as 2μm.
This exactly matches ideal afocal telescope: U_out(x') = M·U_in(M·x').

5000 realizations (4000/500/500), ~1.4s/real → ~2h on A100.

Usage:
    cd /root/dj/D2NN/kim2026 && PYTHONPATH=src python scripts/generate_data_pitch_rescale.py
"""
...
OUT_DIR = Path(__file__).resolve().parent.parent / "data" / "kim2026" / "1km_cn2_5e-14_tel15cm_pitch_rescale"
CACHE_DIR = OUT_DIR / "cache"


def compute_defocus_correction(n, dx_m, device):
    """Analytical defocus: removes beam curvature R≈1000m after M=75 reduction."""
...
def main():
    ...
    print(f"=== Production: Pitch Rescale (delta_n=150μm, NO interpolation) ===")
    ...
    print(f"\nGenerating {N_REALIZATIONS} realizations (ETA ~{N_REALIZATIONS * 1.5 / 3600:.1f}h)")
