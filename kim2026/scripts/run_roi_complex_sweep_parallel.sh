#!/usr/bin/env bash
# Parallel ROI-complex loss sweep: 9 runs in 3 batches of 3
# Each run uses ~9GB VRAM; 3 concurrent fits in ~27GB / 40GB
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

SWEEP_DIR="runs/08_d2nn_beamreducer_roi-complex-sweep_claude"
...
$PYTHON -c "
from scripts.run_d2nn_beamreducer_roi_complex_sweep import build_runs, aggregate_summary
from pathlib import Path
runs = [{'name': r['name'], 'roi_threshold': r['roi_threshold'], 'leakage_weight': r['leakage_weight']}
        for r in build_runs()]
