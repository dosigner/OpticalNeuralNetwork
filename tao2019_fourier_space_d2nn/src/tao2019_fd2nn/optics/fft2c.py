"""Centered FFT utilities used across the project."""
...
def gamma_flip2d(x: torch.Tensor) -> torch.Tensor:
    """Coordinate inversion Γ(x,y)=(-x,-y) on last two axes.

    For centered discrete FFT on even-sized grids, one-pixel cyclic shift
    is required to match F(F(u)) with index-centered inversion.
    """
