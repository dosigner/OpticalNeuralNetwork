...
def _collect_preds(
    ...
    with torch.no_grad():
        for fields, targets in loader:
            ...
            pred = (_I - _min) / (_max - _min).clamp_min(1e-8)
            p = pred.cpu()
            g = targets.cpu()
...
def _sample_triplet(model: torch.nn.Module, loader, device: torch.device) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    model.eval()
    with torch.no_grad():
        for fields, targets in loader:
            fields = fields.to(device)
            ...
            _max = _I.amax(dim=(-2, -1), keepdim=True)
            pred = (_I - _min) / (_max - _min).clamp_min(1e-8)
            return fields[0].detach().cpu(), targets[0].detach().cpu(), pred[0].detach().cpu()
    z = torch.zeros((1, 1))
    return z, z, z
...
def main() -> None:
    ...
    _obj_size = int(_preprocess["resize_to"][0]) if "resize_to" in _preprocess else _N
    _y0 = (_N - _obj_size) // 2
    _x0 = (_N - _obj_size) // 2
    eval_crop_box: tuple[int, int, int, int] | None = None
    if _obj_size < _N:
        eval_crop_box = (_y0, _y0 + _obj_size, _x0, _x0 + _obj_size)
    ...
    history = train_saliency(
        ...
        gamma_flip=bool(cfg["task"].get("gamma_flip", True)),
    ...
    preds, gts = _collect_preds(model, val_loader, device, eval_crop_box=eval_crop_box)
    ...
    field0, gt0, pred0 = _sample_triplet(model, val_loader, device)
