"""Reproduce Tao 2019 Figure 4(b): MNIST classification with batch_size=1024, 30 epochs.
...
Outputs:
  ...
  - fig4b_mnist_bs1024_summary.json
  - fig4b_mnist_bs1024_convergence.png
"""
...
def train_single(config_entry: dict[str, str], base_dir: Path) -> dict[str, Any]:
    ...
    # Append bs1024 tag to experiment name for unique run dirs
    cfg["experiment"]["name"] = cfg["experiment"]["name"] + "_bs1024"
...
def main() -> None:
    ...
    summary_path = base_dir / "fig4b_mnist_bs1024_summary.json"
    with open(summary_path, "w") as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)
    print(f"\nSummary saved: {summary_path}")

    # Plot convergence
    plot_path = base_dir / "fig4b_mnist_bs1024_convergence.png"
    plot_convergence(all_results, plot_path)

    # Final summary table
    print(f"\n{'='*70}")
    print("  Figure 4(b) Results Summary (batch_size=1024)")
    print(f"{'='*70}")
